...
const PrognoseIndex: FunctionComponent = () => {
  const { data: behandlingerSomGårAvVent = [] } = useRestApi<IBehandlingerSomGarAvVentType[]>(
    K9LosApiKeys.HENT_BEHANDLINGER_SOM_GÅR_AV_VENT,
  );

  return (
    <div>
      <BehandlingerGårAvVent behandlingerSomGårAvVent={behandlingerSomGårAvVent} />
    </div>
  );
};
