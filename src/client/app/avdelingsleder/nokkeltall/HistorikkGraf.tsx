...
const fyllInnManglendeDatoerOgSorterEtterDato = (
    ...
    Object.keys(data).reduce((acc, behandlingstype) => {
        ...
        for (let dato = dayjs(periodeStart); dato.isSameOrBefore(periodeSlutt); dato = dato.add(1, 'days')) {
            ...
            koordinater.push(
                funnetDato
                    ? [dayjs(funnetDato.x).format(ISO_DATE_FORMAT), funnetDato.y]
                    : [dato.format(ISO_DATE_FORMAT), 0],
            );
        }
    ...
    return (
        <ReactECharts
            ...
            option={{
                ...
                legend: {
                    ...legendStyle,
                    data: reversertSorterteBehandlingstyper.map((type) =>
                        finnBehandlingTypeNavn(behandlingTyper, type),
                    ),
                },
                grid: gridDef,
                xAxis: [
