...
export const SearchWithDropdown: React.FC<Props> = ({
    ...
    const onChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!suggestionLabel) {
            ...
            setFilteredSuggestions(
                suggestions.filter(
                    (s) =>
                        s.label.toLowerCase().indexOf(suggestionLabel.toLowerCase()) > -1 ||
                        s.value.indexOf(suggestionLabel) > -1,
                ),
            );
    ...
    const deselectSuggestionGroupAndSubValues = (suggestionGroup: string) => {
        ...
        setSelectedSuggestionValues(
            selectedSuggestionValues.filter(
                            ...
                            <fieldset className={styles.fieldset}>
                                ...
                                <ComboboxList className={styles.list}>
                                    {!showFilteredSuggestionsOnly &&
                                        groups
                                            .filter((group) =>
                                                filteredSuggestions.some((suggestion) => suggestion.group === group),
                                            )
                                            .map((group) => {
                                                const suggestionsInThisGroup = suggestions.filter(
                                                ...
                                                return (
                                                    <li key={group}>
                                                        <SelectCheckbox
                                                            value={group}
                                                            label={group}
                                                            onClick={(suggestionGroup) =>
                                                                handleSuggestionGroupToggle(suggestionGroup)
                                                            }
                                                            ...
                                                            <ul>
                                                                {suggestionsInThisGroup.map((suggestion) => (
                                                                    <li
                                                                        ...
                                                                        className={styles.suggestionSubgroup}
                                                                    >
                                                                        <Checkbox
                                                                            className={styles.suggestionCheckbox}
                                                                            value={suggestion.value}
                                                                            onClick={() => onSelect(suggestion.value)}
                                                                            checked={selectedSuggestionValues.includes(
                                                                                suggestion.value,
                                                                            )}
                                                                        >
                                                                            {suggestion.label}
