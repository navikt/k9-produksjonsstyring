...
envsubst '$APP_PORT $APP_HOSTNAME $APP_URL_LOS $APP_NAME $APP_VERSION $AUTH_PROXY_URL $APP_URL' < /etc/nginx/conf.d/app.conf.template > /etc/nginx/conf.d/default.conf
