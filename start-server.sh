...
export AUTH_PROXY_BASE_URL="${AUTH_PROXY_BASE_URL:-http://localhost:8020}"

envsubst '$APP_PORT $APP_HOSTNAME $APP_NAME $K9_LOS_API_URL $AUTH_PROXY_BASE_URL_WSS $AUTH_PROXY_BASE_URL' < /etc/nginx/conf.d/app.conf.template > /etc/nginx/conf.d/default.conf
